          ...
          {activeTab === 'ai' ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              <div>
                ...
                {/* Loading state for auto photo search */}
                {isSearchingPhotos && (
                  <div className="py-3 border border-dashed border-emerald-200 bg-emerald-50/50 rounded-xl flex items-center justify-center gap-2 text-emerald-800 text-xs">
                    <Loader2 size={14} className="animate-spin" />
                    <span>Mencari rekomendasi foto dari web...</span>
                  </div>
                )}

                {/* Horizontal thumbnail selector if photos found */}
                {searchedPhotos.length > 0 && (
                  <div className="space-y-1.5">
                    <p className="text-[11px] text-slate-500 font-medium">Pilih dari hasil pencarian web:</p>
                ...
                {photoUrl && (
                  <div className="relative h-32 w-full rounded-2xl overflow-hidden bg-slate-100 border border-slate-200 shadow-2xs group">
                    <img src={photoUrl} alt="Preview" className="w-full h-full object-cover" />
                    <div className="absolute bottom-2 right-2 bg-black/60 backdrop-blur-md text-white text-[10px] px-2.5 py-0.5 rounded-full font-medium">
                      Foto Terpilih
                    </div>
                  </div>
                )}
