  ...
  try {
    ...
    if (!sql) {
      ...
      return res.status(200).json({
        success: true,
        fromDb: false,
        message: 'DATABASE_URL is not configured yet. Using initial fallback places.',
    ...
    // Return fallback instead of hard 500 crash so frontend never gets 0 places
    return res.status(200).json({
      success: true,
      fromDb: false,
      error: err.message,
